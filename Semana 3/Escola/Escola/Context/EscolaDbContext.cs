...
namespace Escola.Context
{
    public class EscolaDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Usuario>().ToTable("UsuarioTB");
            modelBuilder.Entity<Usuario>().HasKey(x => x.Id).HasName("PK_USUARIO_ID");
            modelBuilder.Entity<Usuario>().Property(x => x.Id).HasColumnName("PK_ID").HasColumnType("INT");
            modelBuilder.Entity<Usuario>().Property(x => x.Nome).IsRequired().HasColumnName("NOME").HasColumnType("VARCHAR").HasMaxLength(150);
            modelBuilder.Entity<Usuario>().Property(x => x.TipoAcesso).IsRequired().HasColumnName("TIPO_ACESSO").HasColumnType("VARCHAR").HasMaxLength(150);
            modelBuilder.Entity<Usuario>().Property(x => x.User).IsRequired().HasColumnName("USUARIO").HasColumnType("VARCHAR").HasMaxLength(150);
            modelBuilder.Entity<Usuario>().Property(x => x.Password).IsRequired().HasColumnName("SENHA").HasColumnType("VARCHAR").HasMaxLength(20);
        }
    }
}
